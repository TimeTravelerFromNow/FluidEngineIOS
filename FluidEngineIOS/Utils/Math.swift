...
// for use in creating orthographicRect camera matrix
struct Rectangle {
  var left: Float = 0
  var right: Float = 0
  var top: Float = 0
...
public func modelSpaceToTexCoord2D( _ modelSpaceCoord: SIMD2<Float>) -> SIMD2<Float> {
    return (float2(modelSpaceCoord.x, -modelSpaceCoord.y) + float2(1,1)) * 0.5
}
...
extension matrix_float4x4 {
    ...
    static func orthographicRect(rect: Rectangle, near: Float, far: Float)-> matrix_float4x4 {
        var result = matrix_identity_float4x4
        result.columns = (
            float4(2 / (rect.right - rect.left), 0, 0, 0),
            float4(0, 2 / (rect.top - rect.bottom), 0, 0),
            float4(0, 0, 1 / (far - near), 0),
            float4((rect.left + rect.right) / (rect.left - rect.right),
...
extension Float {
    ...
    public static var random: Float {
        return Float( arc4random_uniform(0x00FFFFFF)) / 0x00FFFFFF
    }
