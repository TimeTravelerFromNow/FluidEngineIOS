...
class AlienEnemy: GameObject {
    ...
    init(position: float2,
        ...
        _alienRef = LiquidFun.makeAlien(Vector2D(x:position.x,y:position.y), vertices: &boxVertices, vertexCount: boxVertices.count)
...
//        }
    ...
    func updateModelConstants() {
        ...
        LiquidFun.updateBoxButton(_alienRef)
...
//        }
    ...
    func getBoxPositionX() -> Float {
        return Float(LiquidFun.getBoxButtonPosition(_alienRef).x)
    }
    func getBoxPositionY() -> Float {
        return Float(LiquidFun.getBoxButtonPosition(_alienRef).y)
    }
    
    override func getRotationZ() -> Float {
        return LiquidFun.getBoxButtonRotation(_alienRef)
    }
