...
class WaterGun: GameObject {
    ...
    private var _gunRef: UnsafeMutableRawPointer!
...
//        _gunRef = LiquidFun.makeAlien(Vector2D(x:position.x,y:position.y), vertices: &boxVertices, vertexCount: boxVertices.count)
...
//        }
    ...
    func updateModelConstants() {
        ...
        LiquidFun.updateBoxButton(_gunRef)
...
//        }
    ...
    func getBoxPositionX() -> Float {
        return Float(LiquidFun.getBoxButtonPosition(_gunRef).x)
    }
    func getBoxPositionY() -> Float {
        return Float(LiquidFun.getBoxButtonPosition(_gunRef).y)
    }
    
    override func getRotationZ() -> Float {
        return LiquidFun.getBoxButtonRotation(_gunRef)
    }
